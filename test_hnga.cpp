#include "src-ablation/MySolution.h"
#include <iostream>
#include <fstream>
#include <sstream>
#include <vector>
...
extern "C" {
    ...
    // 自适应起点参数
    void set_adaptive_ep_k(int k);
    void set_kmeans_iterations(int iter);
}
...
struct CmdOptsG {
    ...
    int ablate_heap = -1;
    int ablate_reorder = -1;
    int ablate_adaptive_ep = -1;  // 新增
    int adaptive_ep_k = -1;       // 新增
    int kmeans_iterations = -1;   // 新增
...
static CmdOptsG parse_args_g(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (a == "--ablate_reorder" && i+1 < argc) o.ablate_reorder = std::stoi(argv[++i]);
        else if (a == "--ablate_adaptive_ep" && i+1 < argc) o.ablate_adaptive_ep = std::stoi(argv[++i]);
        else if (a == "--adaptive_ep_k" && i+1 < argc) o.adaptive_ep_k = std::stoi(argv[++i]);
        else if (a == "--kmeans_iterations" && i+1 < argc) o.kmeans_iterations = std::stoi(argv[++i]);
...
int main(int argc, char** argv) {
    ...
    logger.writeline(std::string("  Adaptive EP K: ") + std::to_string(opts.adaptive_ep_k));
    logger.writeline("");
    ...
    // 设置自适应起点参数
    if (opts.adaptive_ep_k > 0) {
        set_adaptive_ep_k(opts.adaptive_ep_k);
    }
    if (opts.kmeans_iterations > 0) {
        set_kmeans_iterations(opts.kmeans_iterations);
    }
    ...
    int reorder = (opts.ablate_reorder >= 0) ? opts.ablate_reorder : 0;
    int adaptive_ep = (opts.ablate_adaptive_ep >= 0) ? opts.ablate_adaptive_ep : 0;
    
    set_ablation_flags(prefetch, simd, pruning, heap, reorder, adaptive_ep);
    logger.writeline((std::string("Ablation flags: prefetch=") + std::to_string(prefetch)
