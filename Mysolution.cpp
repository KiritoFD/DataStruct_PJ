...
solution::solution(const std::string& metric_type):metric(metric_type),dim(0){ unsigned int hc=std::thread::hardware_concurrency(); num_threads=(int)(hc>0?hc:1); if(!g_process_start_set){ g_process_start=std::chrono::high_resolution_clock::now(); g_process_start_set=true; } std::cout<<"[solution] hardware_concurrency="<<hc<<", using "<<num_threads<<" threads\n"; }
...
void solution::build(const std::string& base_file){
	...
	if(vectors.empty()){ database.clear(); centroids.clear(); inverted_index.clear(); dim=0; return; }
...
int solution::find_closest_centroid_cached(int vec_idx) const { double min_dist=std::numeric_limits<double>::max(); int best_idx=0; for(size_t c=0;c<centroids.size();++c){ double d=g_vector_to_centroid_distances[vec_idx][c]; if(d<min_dist){ min_dist=d; best_idx=(int)c; } } return best_idx; }

int solution::find_closest_centroid(const std::vector<double>& vec) const { double min_dist=std::numeric_limits<double>::max(); int best_idx=0; for(int i=0;i<(int)centroids.size();++i){ double d=compute_distance(vec,centroids[i]); if(d<min_dist){ min_dist=d; best_idx=i; } } return best_idx; }
...
std::vector<std::pair<int,double>> solution::search(const std::vector<double>& query,int k){
	auto t_search_start=std::chrono::high_resolution_clock::now(); double pre_search_elapsed=std::chrono::duration<double>(t_search_start-g_process_start).count();
	auto close_centroids=find_closest_centroids(query,NPROB);
	std::vector<std::pair<int,double>> all; all.reserve((size_t)std::max(1,g_flat_db.size()*NPROB/NUM_CENTROID));
	int threads_to_use=std::min<int>(num_threads,std::max<int>(1,(int)close_centroids.size())); int chunk_size=((int)close_centroids.size()+threads_to_use-1)/threads_to_use;
	std::vector<std::thread> threads; threads.reserve(threads_to_use); std::vector<std::vector<std::pair<int,double>>> local(threads_to_use); std::vector<double> thread_times(threads_to_use,0.0);
	double query_norm=0.0; if(metric=="cosine"){ for(double v:query) query_norm+=v*v; query_norm=std::sqrt(query_norm); }
	auto worker2=[this,&query,&close_centroids,&local,query_norm,&thread_times](int start,int end,int tid){
		auto t0=std::chrono::high_resolution_clock::now();
		for(int i=start;i<end && i<(int)close_centroids.size();++i){
			int c_id=close_centroids[i].first; auto it=inverted_index.find(c_id); if(it==inverted_index.end()) continue;
			for(int vec_id:it->second){ if(vec_id<g_flat_db.size()){
				const double* dbv=g_flat_db.get_vector(vec_id);
				if(metric=="l2"){ double s=0; for(int d=0;d<dim;++d){ double dd=query[d]-dbv[d]; s+=dd*dd; } local[tid].emplace_back(vec_id,s); }
				else{ double dot=0,dbn=g_flat_db.get_norm(vec_id); for(int d=0;d<dim;++d) dot+=query[d]*dbv[d]; local[tid].emplace_back(vec_id,(query_norm==0||dbn==0)?1.0:1.0-dot/(query_norm*dbn)); }
			} }
		}
		auto t1=std::chrono::high_resolution_clock::now(); thread_times[tid]=std::chrono::duration<double>(t1-t0).count();
	};
	for(int t=0;t<threads_to_use;++t){ int start=t*chunk_size,end=std::min(start+chunk_size,(int)close_centroids.size()); if(start<end) threads.emplace_back(worker2,start,end,t); }
	for(auto&th:threads) th.join();
	for(const auto& lr:local) all.insert(all.end(),lr.begin(),lr.end());
	if(all.size()<= (size_t)k) std::sort(all.begin(),all.end(),[](auto &a,auto &b){return a.second<b.second;}); else { std::partial_sort(all.begin(),all.begin()+k,all.end(),[](auto &a,auto &b){return a.second<b.second;}); all.resize(k); }
