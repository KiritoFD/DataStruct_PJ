...
solution::solution(const std::string& metric_type):metric(metric_type),dim(0), cluster_to_vectors(NUM_CENTROID){ 
...
void solution::build(const std::string& base_file){
    ...
    if(vectors.empty()){ database.clear(); centroids.clear(); inverted_index.clear(); cluster_to_vectors.clear(); cluster_to_vectors.resize(NUM_CENTROID); dim=0; return; }
...
void solution::finalize_build(){
    if(database.empty()||dim==0){ std::cout<<"[solution] finalize_build skipped\n"; centroids.clear(); inverted_index.clear(); cluster_to_vectors.clear(); cluster_to_vectors.resize(NUM_CENTROID); return; }
    ...
    cluster_to_vectors.clear(); 
    cluster_to_vectors.resize(NUM_CENTROID);
    ...
    if(!g_vector_to_centroid_distances.empty() && g_vector_to_centroid_distances.size()==database.size()){
        ...
        for(size_t i=0;i<database.size();++i){
            ...
            #pragma omp critical
            {
                ...
                cluster_to_vectors[best_cluster].push_back(database[i].id);
            }
        }
    } else {
        for(const auto& dp:database){ int c=find_closest_centroid(dp.vec); inverted_index[c].push_back(dp.id); cluster_to_vectors[c].push_back(dp.id); }
    }
...
std::vector<std::pair<int,double>> solution::search(const std::vector<double>& query, int k) {
    ...
    for (const auto& centroid_info : close_centroids) {
        ...
        if (c_id < cluster_to_vectors.size()) {
            estimated_size += cluster_to_vectors[c_id].size();
        }
    }
    ...
    auto worker = [this, query_ptr, &close_centroids, &local_results, query_norm](int start, int end, int tid) {
        ...
        for (int i = start; i < end && i < (int)close_centroids.size(); ++i) {
            ...
            if (c_id >= cluster_to_vectors.size()) continue;
            
            const std::vector<int>& cluster_vectors = cluster_to_vectors[c_id];
