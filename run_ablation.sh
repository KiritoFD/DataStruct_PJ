...
WORKDIR=$(pwd)
BINARY=${BINARY:-"hnga"}  # 默认使用 hnga，可通过环境变量覆盖
...
EFS=${EFS:-432}
THREADS=${THREADS:-32}
ADAPTIVE_EP_K=${ADAPTIVE_EP_K:-256}
...
HYPER_EFS_MIN=${HYPER_EFS_MIN:-1200}
...
OUT_DIR="result-ablation/ablation_${TS}"
mkdir -p "$OUT_DIR"
CSV="$OUT_DIR/results.csv"

# CSV header - 添加 adaptive_ep 列
...
# define variants: name,csr,prefetch,simd,pruning,heap,flat_index,reorder,adaptive_ep
# 说明：
#   - 0 = 启用该优化 (baseline)
#   - 1 = 禁用该优化 (ablation)
...
variants=(
  ...
  "add_adaptive_ep,0,0,0,0,0,0,0,1"
  "dynamic_with_opts,0,0,0,0,0,1,0,0"
)
...
run_variant() {
  ...
  IFS=',' read -r varname csr prefetch simd pruning heap flat_index reorder adaptive_ep <<< "$variant_spec"
  ...
  for run_index in $(seq 1 $REPEATS); do
    ...
    ./"$BINARY" --base "$BASE_FILE" --query "$QUERY_FILE" --truth "$TRUTH_FILE" --k "$K" \
      ...
      --ablate_reorder "$reorder" --ablate_adaptive_ep "$adaptive_ep" \
      --adaptive_ep_k "$ADAPTIVE_EP_K" > "$log_file_stdout" 2>&1 || {
...
export BASE_FILE QUERY_FILE TRUTH_FILE K THREADS REPEATS ADAPTIVE_EP_K BINARY
...
# Collect results from all meta files into CSV - 更新以包含 adaptive_ep
find "$OUT_DIR" -name "meta_*.txt" | sort | while read -r meta_file; do
  ...
  # Parse variant_spec - 添加 adaptive_ep
  IFS=',' read -r varname csr prefetch simd pruning heap flat_index reorder adaptive_ep <<< "$variant_spec"
  ...
  # Write CSV line - 添加 adaptive_ep 字段
  printf "%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n" \
    "$varname" "$csr" "$prefetch" "$simd" "$pruning" "$heap" "$flat_index" "$reorder" "$adaptive_ep" \
