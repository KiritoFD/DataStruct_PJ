...
int main(int argc, char** argv) {
    ...
    logger.writeline(std::string("  M (CLI): ") + std::to_string(opts.M));
    logger.writeline(std::string("  Max layer (CLI): ") + std::to_string(opts.max_layer));
    logger.writeline(std::string("  EFC (CLI): ") + std::to_string(opts.efc));
    logger.writeline(std::string("  EFS (CLI): ") + std::to_string(opts.efs));
    logger.writeline(std::string("  Threads (CLI): ") + std::to_string(opts.threads));
    logger.writeline(std::string("  Debug: ") + std::to_string(opts.debug));
    logger.writeline("");

    // Always call set_hnsw_params — it will normalize -1 -> defaults
    set_hnsw_params(opts.M, opts.max_layer, opts.efc, opts.efs, opts.threads);

    // Query the effective parameters
    int eff_M=0, eff_ML=0, eff_EFC=0, eff_EFS=0, eff_threads=0;
    get_hnsw_params(&eff_M, &eff_ML, &eff_EFC, &eff_EFS, &eff_threads);

    // Log effective parameters
    logger.writeline("\nConfiguration (effective):");
    logger.writeline(std::string("  M: ") + std::to_string(eff_M));
    logger.writeline(std::string("  Max layer: ") + std::to_string(eff_ML));
    logger.writeline(std::string("  EFC: ") + std::to_string(eff_EFC));
    logger.writeline(std::string("  EFS: ") + std::to_string(eff_EFS));
    logger.writeline(std::string("  Threads: ") + std::to_string(eff_threads));
    logger.writeline("");
    
    // Verify: if user supplied a positive CLI value, the effective value should match
    if (opts.M > 0 && eff_M != opts.M) {
        logger.writeline(std::string("WARNING: CLI M=") + std::to_string(opts.M)
                         + " but effective M=" + std::to_string(eff_M) + " (normalized)");
    }
    if (opts.max_layer > 0 && eff_ML != opts.max_layer) {
        logger.writeline(std::string("WARNING: CLI MaxLayer=") + std::to_string(opts.max_layer)
                         + " but effective MaxLayer=" + std::to_string(eff_ML) + " (normalized)");
    }
    if (opts.efc > 0 && eff_EFC != opts.efc) {
        logger.writeline(std::string("WARNING: CLI EFC=") + std::to_string(opts.efc)
                         + " but effective EFC=" + std::to_string(eff_EFC) + " (normalized)");
    }
    if (opts.efs > 0 && eff_EFS != opts.efs) {
        logger.writeline(std::string("WARNING: CLI EFS=") + std::to_string(opts.efs)
                         + " but effective EFS=" + std::to_string(eff_EFS) + " (normalized)");
    }
    if (opts.threads > 0 && eff_threads != opts.threads) {
        logger.writeline(std::string("WARNING: CLI Threads=") + std::to_string(opts.threads)
                         + " but effective Threads=" + std::to_string(eff_threads) + " (normalized)");
    }
