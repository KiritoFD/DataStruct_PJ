...
// Forward declare FlatHNSW and HnswGraphBuilder
struct FlatHNSW;
...
// 【修改】版本5：增加二进制指纹存储
static const uint32_t INDEX_VERSION = 5u;

static void generate_dfs_reordering(HnswGraphBuilder* src, std::vector<int>& old_to_new, std::vector<int>& new_to_old) {
    int N = src->size();
...
static bool save_flat_index(FlatHNSW* flat, const std::string& path) {
    ...
    // [新增] 写入二进制指纹（版本5+）
    uint64_t signatures_size = flat->signatures.size();
    ofs.write((char*)&signatures_size, sizeof(signatures_size));
    if (signatures_size > 0) {
        ofs.write((char*)flat->signatures.data(), signatures_size * sizeof(uint64_t));
    }
...
static FlatHNSW* load_flat_index(const std::string& path) {
    ...
    if (version < 1 || version > INDEX_VERSION) return nullptr;  // 支持版本1-5
    ...
    if (version >= 5) {
        ...
        ifs.read((char*)&signatures_size, sizeof(signatures_size));
        if (signatures_size > 0) {
            flat->signatures.resize(signatures_size);
            ifs.read((char*)flat->signatures.data(), signatures_size * sizeof(uint64_t));
        }
    } else {
        // 旧版本缓存：需要重新生成指纹
        flat->signatures.resize((size_t)flat->num_nodes * 2);
        for (int i = 0; i < flat->num_nodes; ++i) {
            flat->compute_signature(i, flat->get_vec(i));
        }
        if (DEBUG_TIMING) {
            std::cout << "[Cache] Regenerated binary signatures for old version cache." << std::endl;
        }
    }
...
static FlatHNSW* convert_to_flat(HnswGraphBuilder* src) {
    ...
    // [新增] 生成所有向量的二进制指纹
    flat->signatures.resize((size_t)N * 2);
    for (int new_id = 0; new_id < N; ++new_id) {
        flat->compute_signature(new_id, flat->data.data() + (size_t)new_id * flat->dim);
    }
    
    if (DEBUG_TIMING) {
        std::cout << "[FlatConvert] Generated binary signatures for " << N << " nodes." << std::endl;
        std::cout.flush();
    }
    ...
    if (DEBUG_TIMING) {
        std::cout << "[FlatHNSW-CSR] Converted " << N << " nodes with binary fingerprints." << std::endl;
    }
